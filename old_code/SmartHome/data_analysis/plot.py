...
lstm_te = np.load("./LSTM/HomeB/threshold0-2-st10-lr0001-batch16_te_acc.npy")[:300, ...]
fcn_tr = np.load("./Fully_connected/Home B/threshold0-2-st10-lr0001-batch16_tr_acc.npy")[:300, ...]
lstm_tr = np.load("./LSTM/HomeB/threshold0-2-st10-lr0001-batch16_tr_acc.npy")[:300, ...]
# homeA[0, 1] = 0.45
...
# plt.plot(lstm_tr[:, 0], smooth_data(lstm_tr[:, 1], 0.9), 'c-', label='Train on Home B (LSTM)', linewidth=3.0)
plt.plot(fcn_te[:, 0], smooth_data(fcn_te[:, 1], 0.9), 'y-', label="Test on Home B (FCN)", linewidth=3.0)
plt.plot(lstm_te[:, 0], smooth_data(lstm_te[:, 1], 0.9), 'g-', label='Test on Home B (LSTM)', linewidth=3.0)
...
plt.ylim(0, 1)
# plt.title("Home B")
plt.title("Test on Home B with FCN and LSTM")
